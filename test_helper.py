...
def get_file_output(encoding="utf-8", path=sys.argv[-1], arg_string=""):
    ...
    return list(
        map(lambda x: str(x.decode(encoding)), proc.communicate()[0].splitlines())
    )
        ...
        if len(sample) == 2:
            ...
            if "\n".join(output) != sample[1]:
                failed(
                    ...
                    "\n \n"
                    "Your result:\n{}".format(
                        str.strip(sample[0]), str.strip(sample[1]), "\n".join(output)
                    )
                )
                return
        set_congratulation_message(
            "All test from samples passed. Now we are checking your solution on Stepik server."
        )
